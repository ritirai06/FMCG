...
class OrderListService
{
    ...
    {
        $user = Auth::user();
        $query = Order::with(['store', 'createdBy', 'assignedDelivery', 'assignedDeliveryPerson', 'items.product']);
    ...
    {
        // if no authenticated user, treat as admin (public listing)
        if (!$user) {
            return $query;
        }
        if ($user->isAdmin()) {
    ...
    public function getOrdersSummary(): array
    {
        ...
        // Apply role filter (skip if unauthenticated)
    ...
    public function getStoresForFilter(): Collection
    {
        ...
        // unauthenticated users see all stores (like admin)
        ...
        return new Collection(); // Empty for delivery
    }
