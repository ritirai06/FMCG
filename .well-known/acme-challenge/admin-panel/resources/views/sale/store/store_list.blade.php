                ...
                <div class="col-xl-12 col-xxl-12">
                    <div class="card store-insights-card">
                        <div class="card-header border-0 pb-2">
                            <h4 class="card-title mb-0">Store List Insights</h4>
                        </div>
                        <div class="card-body">
                            @php
                                $allStores = collect($stores ?? []);
                                $assignedLocalityStores = $allStores->filter(function ($s) {
                                    return \Illuminate\Support\Str::contains((string)($s->address ?? ''), 'Locality:');
                                });
                                $searchTerm = trim((string) request('search', ''));
                                $statusFilter = request('status', 'all');
                                $activeStores = $allStores->filter(function ($s) {
                                    $status = strtolower((string) ($s->status ?? ''));
                                    return (int) ($s->status ?? 0) === 1 || $status === 'active';
                                });
                                $inactiveStores = $allStores->filter(function ($s) {
                                    $status = strtolower((string) ($s->status ?? ''));
                                    return (int) ($s->status ?? 0) === 0 || $status === 'inactive';
                                });
                                $visibleStores = $searchTerm === ''
                                    ? $allStores
                                    : $allStores->filter(function ($s) use ($searchTerm) {
                                        $needle = \Illuminate\Support\Str::lower($searchTerm);
                                        $name = \Illuminate\Support\Str::lower((string) ($s->store_name ?? ''));
                                        $code = \Illuminate\Support\Str::lower((string) ($s->code ?? ''));
                                        return \Illuminate\Support\Str::contains($name, $needle) || \Illuminate\Support\Str::contains($code, $needle);
                                    });
                            @endphp

                            <div class="row g-3 mb-3">
                                <div class="col-md-4">
                                    <div class="store-insights-metric">
                                        <div class="metric-label">Total</div>
                                        <div class="metric-value">{{ $allStores->count() }}</div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="store-insights-metric">
                                        <div class="metric-label">Active</div>
                                        <div class="metric-value">{{ $activeStores->count() }}</div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="store-insights-metric">
                                        <div class="metric-label">Inactive</div>
                                        <div class="metric-value">{{ $inactiveStores->count() }}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row g-3">
                                <div class="col-lg-5">
                                    <div class="store-insights-subcard">
                                        <h6 class="mb-2">Insights Summary</h6>
                                        <div class="table-responsive">
                                            <table class="table table-sm store-insights-table mb-0">
                                                <tbody>
                                                    <tr>
                                                        <th style="width:46%;">Assigned locality stores</th>
                                                        <td>{{ $assignedLocalityStores->count() }} / {{ $allStores->count() }}</td>
                                                    </tr>
                                                    <tr>
                                                        <th>Search + Filter</th>
                                                        <td>
                                                            Search: <strong>{{ $searchTerm !== '' ? $searchTerm : 'Not set' }}</strong><br>
                                                            Status: <strong>{{ ucfirst((string) $statusFilter) }}</strong>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <th>View details</th>
                                                        <td><i class="fas fa-eye text-info"></i> icon se detail popup open hoga.</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                    ...
                                    <div class="store-insights-subcard">
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-sm store-insights-table mb-0">
                                                ...
                                                <tbody>
                                                    @forelse($visibleStores->take(8) as $st)
                                                        <tr>
                                                            <td>{{ $st->store_name ?? 'N/A' }}</td>
                                                            <td><small>{{ $st->code ?? '-' }}</small></td>
                                                        </tr>
                                                    @empty
                                                        <tr>
                                                            <td colspan="2" class="text-muted text-center">No stores found</td>
                                                        </tr>
                                                    @endforelse
                                                </tbody>
                                            </table>
